...
export default function Home() {
  ...
  async function search() {
    ...
    setDone(true);
    try {
      // push the term into the URL so it can be restored when navigating back
  ...
  // run search automatically when page loads if q param exists
  useEffect(() => {
    const initial = searchParams.get("q") || "";
    if (initial && initial !== q) {
      setQ(initial);
      // perform search using the initial value
      (async () => {
        setDone(false);
        const res = await fetch(`${API}/api/search?q=${encodeURIComponent(initial)}&k=20`);
        const data = await res.json();
        setItems(data.results || []);
        setDone(true);
      })();
